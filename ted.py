        ...
        try:
            ...
            try:
                if response.json()['status_code'] == 0:
                    _lock.acquire()
                    print(Colorate.Horizontal(Colors.purple_to_red, f'TikTok View by Tezos^| success: {success} fails: {fails} reqs: {reqs} rps: {rps} rpm: {rpm}'))
                    success += 1
                    _lock.release()
            except:
...
if __name__ == "__main__":
    ...
    set_title("Tool Buff View Tik Tok By Tezos")
    txt = """\n\n████████╗███████╗███████╗ ██████╗ ███████╗
╚══██╔══╝██╔════╝╚══███╔╝██╔═══██╗██╔════╝
   ██║   █████╗    ███╔╝ ██║   ██║███████╗
   ██║   ██╔══╝   ███╔╝  ██║   ██║╚════██║
   ...
   ╚═╝   ╚══════╝╚══════╝ ╚═════╝ ╚══════╝   \n"""
    print(
        Colorate.Vertical(
            Colors.DynamicMIX((Col.light_green, Col.blue)), Center.XCenter(txt)
        )
    )
    
    try:
        link = str(Write.Input("\n\n            ? - Nhập Link Video TikTok > ", Colors.white_to_red, interval=0.0001))
        __aweme_id = str(
        ...
        os.system("cls" if os.name == "nt" else "clear")
        input(Col.red + "x - Liên kết không hợp lệ, hãy thử chỉ nhập id video" + Col.reset)
        sys.exit(0)
